...
const char* ModelException::what() const noexcept
{
	...
	oss << DirectException::what() << std::endl
		<< "[Note] " << GetNote();
	whatBuffer = oss.str();
...
{
	return "Modular Model Exception";
}
...
{
	//if there is no selected node, set selectedid to an impossible valuereal
...
{
	Assimp::Importer imp;
	const auto pScene = imp.ReadFile(cwd + "Assets\\Models\\" + fileName.c_str(),
...
{
	namespace dx = DirectX;
	using Modular::VertexLayout;

	Modular::VertexBuffer vbuf(std::move(
		VertexLayout{}
	...
	if (mesh.mMaterialIndex >= 0)
	{
		...
		const auto base = cwd + "Assets\\Models\\nano_textured\\"s;
	...
	auto pvs = std::make_unique<VertexShader>(gfx, StringConverter::StringToWide(cwd + "TexturedPhongVS.cso"));
	auto pvsbc = pvs->GetBytecode();
	bindablePtrs.push_back(std::move(pvs));

	if (hasSpecularMap) {
		bindablePtrs.push_back(std::make_unique<PixelShader>(gfx, StringConverter::StringToWide(cwd + "PhongPSSpecMap.cso")));
	}
	else
	{
		bindablePtrs.push_back(std::make_unique<PixelShader>(gfx, StringConverter::StringToWide(cwd + "TexturedPhongPS.cso")));
	}
