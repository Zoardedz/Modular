	...
	TestObject(gfx, rng, adist, ddist, odist, rdist)
{
	...
	{
		auto pvs = std::make_unique<VertexShader>(gfx, StringConverter::StringToWide(std::string(cwd + "PhongVS.cso")));
		auto pvsbc = pvs->GetBytecode();
		AddStaticBind(std::move(pvs));

		AddStaticBind(std::make_unique<PixelShader>(gfx, StringConverter::StringToWide(std::string(cwd + "IndexedPhongPS.cso"))));
